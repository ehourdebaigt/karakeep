...
export interface QuickAddFormProps {
  // Full control over the outer <form>'s classes - callers own whether it
  // looks like a card (bg/border/rounded/fixed height) or sits bare inside
  // something else (e.g. a Dialog, which already provides that chrome).
  className?: string;
  // Optional content rendered above the textarea (e.g. the "New Item" +
  // ⌘E hint row on the inline card). Omitted entirely for the dialog
  // variant since the Dialog's own title covers that.
  header?: React.ReactNode;
  // Grows the textarea with its content up to half the viewport height.
  autoResize?: boolean;
  // Lets the user manually drag-resize the textarea.
  allowManualResize?: boolean;
  // Focuses the textarea as soon as this mounts (used by the dialog).
  autoFocus?: boolean;
...
export function QuickAddForm({
  ...
  useEffect(() => {
    ...
    // Only run on mount - autoFocus is not expected to change afterwards.
